---
name: softeng
description: Run softeng commands
...
- **`scripts/softeng.sh`** — Main entry point for executing uspecs commands

## Execution instructions

- Parse user input as {action} [options] {other-input} like `uc2 --no-impl here is some prompt`
- run `bash scripts/softeng.sh action {action} {options}` and follow the instructions in the output how to process {other-input}
